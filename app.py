...
def page2_company_dashboard():
    ...
    with col1:
        ...
        # 부서 목록 조회 (서버 API 사용)
        departments = get_departments_by_company(st.session_state.selected_company)
        
        # 검색어로 필터링
        if search_query:
            departments = [
                dept for dept in departments 
                if search_query.lower() in dept.get("dept_name", "").lower()
            ]
        ...
        if departments:
            for dept in departments:
                ...
                # 부서 설명은 Supabase에서 직접 가져오기
                try:
                    dept_info = supabase.table("department").select("dept_desc").eq("dept_id", dept_id).execute()
                    if dept_info.data and len(dept_info.data) > 0:
                        dept_desc = dept_info.data[0].get("dept_desc", "")
                    else:
                        dept_desc = ""
                except Exception as e:
                    print(f"부서 설명 조회 오류: {e}")
                    dept_desc = ""
    ...
    with col2:
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        recent_cs = get_recent_cs_messages(limit=5)
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        most_assigned = get_most_assigned_cs(limit=5)
...
def page3_department_dashboard():
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    cs_queue = get_department_cs_queue(st.session_state.selected_dept_id)
    ...
    # 통계 대시보드
    stats = get_department_stats(st.session_state.selected_dept_id)
