...
# Streamlit Cloud의 Secrets 또는 환경 변수에서 서버 URL 가져오기
# Streamlit Cloud에서는 st.secrets를 사용, 로컬에서는 환경 변수 사용
try:
    # Streamlit Cloud Secrets 사용
    if hasattr(st, 'secrets') and 'SERVER_URL' in st.secrets:
        server_url = st.secrets['SERVER_URL']
    else:
        # 환경 변수에서 가져오기 (로컬 개발 또는 일반 환경 변수)
        server_url = os.getenv("SERVER_URL", "http://54.180.121.208:8000")
except:
    # fallback: 환경 변수 사용
    server_url = os.getenv("SERVER_URL", "http://54.180.121.208:8000")
